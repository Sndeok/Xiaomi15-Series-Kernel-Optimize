...
- With-NTSync 说明：`ntsync.ko` 依赖高通 / QCOM 的 `debug_symbol` 前置模块；使用 `lsmod | grep -E "ntsync|kshrink|mi_rmap|binder_prio"` 查询时，`debug_symbol` 因 Used by 列包含 `ntsync` 等模块而被匹配显示，属于正常现象。
