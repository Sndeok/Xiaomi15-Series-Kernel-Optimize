...
- With-NTSync 说明：`ntsync.ko` 依赖高通 / QCOM 的 `debug_symbol` 前置模块；使用 `lsmod | grep -E "^(Module|ntsync|mi_async_reclaim|binder_prio|mi_rmap_efficiency|kshrink_slabd|debug_symbol)[[:space:]]"` 查询时可能显示类似下面的结果，该命令按模块名首列筛选；`debug_symbol` 是 `ntsync.ko` 的前置依赖，显示出来属于正常现象。
